...
class Component implements Component_Interface, Templating_Component_Interface
{
   ...
   * @return false|string
   */
  public function inline_svg($id, $comment = false): false|string
  {
    if (!$id) {
      return false;
    }
    ...
    if (preg_match_all('#id="([^"]+)"#si', $svg, $m)) {
      foreach ($m[1] as $id_attr) {
        $id_new = $id_attr . '--' . $i;
        $svg = str_replace($id_attr, $id_new, $svg);
      }
    }
  ...
  public function get_adjacent_post_link($post_type, $previous = true): array
  {
   ...
   * @return string|false
   */
  public function youtube_id($url): string|false
  {
    $id = false;
   ...
   * @return string|false
   */
  public function youtube_short_id($url): string|false
  {
    $id = false;
   ...
   * @return string
   */
  public function handle_attributes($attrs): string
  {
