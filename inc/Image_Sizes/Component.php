...
class Component implements Component_Interface
{
   ...
   * @param array $size Image size. Accepts an array of width and height values.
  ...
  {
    if (!empty($attr['sizes'])) {
      $old_sizes_attr = 'sizes="' . esc_attr($attr['sizes']) . '"';
      $html = str_replace($old_sizes_attr, 'sizes="100vw"', $html);
    }
   ...
   * @param string|array $size Registered image size name or flat array of dimensions.
   * @return array The filtered attributes for the image markup.
   */
  public function filter_post_thumbnail_sizes_attr(array $attr, WP_Post $attachment, string|array $size): array
  {

    if (in_array($size, array('thumbnail', 'medium', 'avatar'), true)) {
      return $attr;
    }

    if (is_array($size) && ($size[0] ?? 0) < 360) {
      return $attr;
    }
